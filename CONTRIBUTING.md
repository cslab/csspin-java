...
The csspin-java repository is hosted on a private GitLab instance, while the
repository itself is mirrored to [GitHub](https://github.com/cslab/csspin-java)
...
Please follow the guidelines below when contributing to the csspin-java project.
...
repository](https://github.com/cslab/csspin-java/issues) for reporting issues
while CONTACT Software Group GmbH internal contributors can use the GitLab issue
tracker.
...
3. `git clone <your-fork-or-repo-url> csspin-java && cd csspin-java && pre-commit install`
...
> This section is only relevant for maintainers of the csspin-java project.

The version scheme used is major.minor.patch following semver.org.

Releases are created by csspin-java maintainers within the GitLab instance
...
1. Create the release notes by opening a new merge request at the csspin-java
   repository in GitLab targeting the master branch and adding a new section to
   the `doc/relnotes.rst` file. Follow the instructions given in that file
   regarding the format of the release notes. Once the merge request is reviewed
   and approved, merge it into master.
